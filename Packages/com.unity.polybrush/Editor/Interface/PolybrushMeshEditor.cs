...
namespace UnityEditor.Polybrush
{
    ...
    public class PolybrushMeshEditor : Editor
    {
        ...
        void DrawExtraActions()
        {
            PolybrushMesh polybrushmesh = target as PolybrushMesh;
            MeshRenderer mr = polybrushmesh.gameObject.GetComponent<MeshRenderer>();
            FurMeshForPolyBrush fur = polybrushmesh.gameObject.GetComponent<FurMeshForPolyBrush>();
            using (new EditorGUILayout.HorizontalScope())
            {
                ...
                {
                    if (GUILayout.Button("Export Mesh and Convert to FurMesh"))
                    {
                        PolyEditorUtility.SaveMeshAsset(polybrushmesh.polyMesh.ToUnityMesh(), null, null, -1, "", (path) =>
                        {
                            Mesh savedMesh = AssetDatabase.LoadAssetAtPath<Mesh>(path);
                            var go = GameObject.CreatePrimitive(PrimitiveType.Quad);
                            DestroyImmediate(go.GetComponent<Collider>());
                            go.GetComponent<MeshFilter>().sharedMesh = savedMesh;
                            go.GetComponent<Renderer>().sharedMaterials = mr.sharedMaterials;
                            go.transform.position = polybrushmesh.gameObject.transform.position;
                            go.transform.rotation = polybrushmesh.gameObject.transform.rotation;
                            go.transform.localScale = polybrushmesh.gameObject.transform.localScale;
                            go.name = savedMesh.name;
                            var newFur = go.AddComponent<FurMesh>();
                            newFur.height = fur.height;
                            newFur.slice = fur.slice;
                            newFur.threshodCurve = fur.threshodCurve;
                            newFur.tiltingUV1 = fur.tiltingUV1;
                            newFur.triangleCull = fur.triangleCull;
                            polybrushmesh.gameObject.transform.parent = go.transform;
                            polybrushmesh.gameObject.SetActive(false);
