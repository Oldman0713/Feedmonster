...
namespace UnityEditor.Polybrush
{
    ...
    internal class PolybrushEditor : ConfigurableWindow
    {
        ...
        public static bool drawFogsAngle = false;
        bool DoMeshRaycast(Ray mouseRay, BrushTarget target, MirrorSettings mirrorSettings)
        {
            ...
            foreach (Ray ray in s_Rays)
            {
                ...
                if (PolySceneUtility.WorldRaycast(ray, editable.transform, editable.visualMesh, out hit))
                {
                    ...
                    hitMesh = true;
                    drawFogsAngle = false;
                    #region    Treeverse Extension
                    fur = editable.transform.GetComponent<FurMeshForPolyBrush>();
                    var ren = editable.transform.GetComponent<Renderer>();

                    bool hasFurMaterial = false;
                    //Debug.Log(brushSettings.name);
                    foreach (var m in ren.sharedMaterials)
                    {
                        if (m.shader.name == "Treeverse/Static/Environment/HeightBlend")
                        {
                            hasFurMaterial = true;
                            break;
                        }
                        if (m.shader.name == "Treeverse/VFX/AreaFog" && tool == BrushTool.Texture)
                        {
                            drawFogsAngle = true;
                            break;
                        }
                    ...
                    if (hasFurMaterial && fur == null)
                    {
                        fur = editable.transform.gameObject.AddComponent<FurMeshForPolyBrush>();
                    }
                    if (fur)
                    {
                        if (fur.enabled)
