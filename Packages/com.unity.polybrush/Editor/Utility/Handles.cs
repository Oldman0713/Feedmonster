...
namespace UnityEditor.Polybrush
{
    ...
    {
        static void DrawFogArrow(Vector3 p, BrushSettings brushSettings)
        {
            if (PolybrushEditor.drawFogsAngle)
            {
                Handles.color = Color.red;
                var rad = brushSettings.strength * 360.0f * Mathf.Deg2Rad;
                Vector3 arrowForward = -new Vector3(-Mathf.Cos(rad), 0.0f, Mathf.Sin(rad)) * HandleUtility.GetHandleSize(p);
                Vector3 tri_0 = p + arrowForward * 2.0f;
                Vector3 tri_1 = tri_0 - (Vector3.Cross(arrowForward, Vector3.up) + arrowForward) * 0.5f;
                Vector3 tri_2 = tri_0 - (Vector3.Cross(-arrowForward, Vector3.up) + arrowForward) * 0.5f;
                Handles.DrawLine(p, tri_0);
                Handles.DrawLine(tri_0, tri_1);
                Handles.DrawLine(tri_0, tri_2);
            }
        }
